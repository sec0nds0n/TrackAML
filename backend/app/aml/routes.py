...
def cases_page():
    ...
    user_map = {
        u['id']: u['username'] if u['username'].lower() == u['role'].lower()
        else f"{u['role'].capitalize()} – {u['username']}"
    ...
    for c in all_cases:
        for a in c['assignments']:
            a['role'] = user_map.get(int(a['user_id']), 'Unknown')

        if can_user_see_case(c, user):
            visible_cases.append(c)

    return render_template('cases.html', cases=visible_cases, users=users)

@aml_bp.route('/cases/<int:case_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_case(case_id):
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    
    cur.execute("SELECT unnest(enum_range(NULL::case_status_enum))")
    status_options = [row[0] for row in cur.fetchall()]
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
            ...
            SET description = %s,
                ...
                updated_at = NOW()
            WHERE id = %s
        """, (description, status, reason, source, tag_list, case_id))

        conn.commit()
        cur.close()
        conn.close()
        flash("✅ Case updated", "success")
        return redirect(url_for('aml.cases_page'))
    ...
    return render_template('edit_case.html', case=case, status_options=status_options)

@aml_bp.route('/cases/<int:case_id>/assign', methods=['POST'])
def assign_case(case_id):
    user_id = request.form.get('user_id')
    if not user_id:
        flash("❌ Pilih user terlebih dahulu.", "danger")
        return redirect(url_for('aml.cases_page'))
    ...
    flash(f"✅ Case {case_id} berhasil di-assign ke user {user_id}", "success")
    return redirect(url_for('aml.cases_page'))

@aml_bp.route('/cases/<int:case_id>')
@login_required
def view_case(case_id):
    from app.services.case_service import get_case_with_assignments, can_user_see_case
    from app.services.user_service import get_all_users
    ...
    # Ambil nama/role untuk assignments
    users = get_all_users()
    user_map = {
        u['id']: u['username'] if u['username'].lower() == u['role'].lower()
        else f"{u['role'].capitalize()} – {u['username']}"
    ...
    for a in case['assignments']:
        a['label'] = user_map.get(int(a['user_id']), 'Unknown')

    return render_template('case_detail.html', case=case)

# Anomaly Transactions
@aml_bp.route('/anomalies/transactions')
def anomaly_transactions_page():
    anomalies = get_anomaly_transactions(limit=500)  # atau limit sesuai kebutuhan
    cases = list_cases(limit=1000)
    cased = set(c['reference_id'] for c in cases)
    
    # Filter anomaly yang belum dibuatkan case
    pending = [tx for tx in anomalies if tx['tx_hash'] not in cased]

    return render_template(
        'anomaly_transactions.html',
        pending=pending
    )

@aml_bp.route('/anomalies/wallets')
def anomaly_wallets_page():
    blacklisted_wallets = get_blacklisted_wallets()
    cases = list_cases(limit=1000)
    cased = set(c['reference_id'] for c in cases)

    pending = [w for w in blacklisted_wallets if w['address'] not in cased]

    return render_template(
        'anomaly_wallets.html',
        pending=pending
    )

@aml_bp.route('/cases/<int:case_id>/report')
def generate_case_report(case_id):
    case = get_case_with_assignments(case_id)
    if not case:
        return "Case not found", 404
    rendered = render_template(
        'aml/case_report.html',
        case=case,
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=case_{case_id}_report.pdf'
    return response

@aml_bp.route('/cases/create_transaction', methods=['POST'])
def create_case_transaction():
    tx_hash = request.form.get('tx_hash')
    try:
        create_case('transaction', tx_hash)
        flash('Case created for transaction!', 'success')
    except Exception as e:
        flash(str(e), 'danger')
    return redirect(url_for('aml.anomaly_transactions_page'))

@aml_bp.route('/cases/create_wallet', methods=['POST'])
def create_case_wallet():
    wallet_address = request.form.get('wallet_address')
    try:
        create_case('wallet', wallet_address)
        flash('Case created for wallet!', 'success')
    except Exception as e:
        flash(str(e), 'danger')
    return redirect(url_for('aml.anomaly_wallets_page'))