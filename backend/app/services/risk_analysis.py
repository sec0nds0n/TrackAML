...
def get_high_risk_address_count():
    ...
    cur = conn.cursor()
    try:
        # robust: case-insensitive & toleran spasi
        cur.execute("""
            SELECT COUNT(*) 
            FROM wallet_risk 
            WHERE LOWER(TRIM(risk_profile)) = LOWER(TRIM(%s));
        """, ('High Risk',))
...
def get_wallet_risk_distribution():
    """
    Ambil distribusi risk wallet berdasarkan risk_profile di tabel wallet_risk.
    Bucket: critical / high / medium. Sisanya diabaikan untuk kartu ini.
    """
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        WITH w AS (
            SELECT DISTINCT address, CAST(risk_profile AS TEXT) AS rp
            FROM wallet_risk
            WHERE risk_profile IS NOT NULL
        )
        SELECT
            COUNT(*)                                             AS total,
            COUNT(*) FILTER (WHERE LOWER(rp) IN ('critical','very high','severe'))         AS critical,
            COUNT(*) FILTER (WHERE LOWER(rp) LIKE 'high%')                                  AS high,
            COUNT(*) FILTER (WHERE LOWER(rp) LIKE 'medium%')                                AS medium
        FROM w;
    """)
    row = cur.fetchone()
    cur.close(); conn.close()

    total, critical, high, medium = [int(x or 0) for x in row]
    def pct(x): 
