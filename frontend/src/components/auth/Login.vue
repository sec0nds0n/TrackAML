...
// Predefined user roles and permissions (fallback for demo)
const userRoles = {
    'admin@example.com': {
        role: 'admin',
...
// Determine user role based on API response or email
const determineUserRole = (apiResponse, userEmail) => {
    // If API response includes role information, use it
    ...
    // If API response includes permissions directly
    if (apiResponse.role || apiResponse.permissions) {
        return {
    ...
    // Fallback to predefined roles based on email
    const fallbackRole = userRoles[userEmail];
    if (fallbackRole) {
        return fallbackRole;
    }
    
    // Default role if nothing matches
    return {
        role: 'user',
        permissions: ['read']
    };
...
const login = async () => {
    ...
    try {
        ...
        // Add remember me if needed by API
        if (rememberMe.value) {
        ...
        // Call API
        const apiResponse = await loginWithAPI(credentials);
        
        console.log('Login successful:', apiResponse);
        
        // Extract user information and token
        const token = apiResponse.token || apiResponse.access_token || apiResponse.accessToken;
        ...
        const userRole = determineUserRole(apiResponse, email.value);
        
        // Prepare user data for store
        const userData = {
            email: email.value,
            name: user.name || user.full_name || email.value.split('@')[0],
            role: userRole.role,
            permissions: userRole.permissions,
            token: token,
            ...user // Include any additional user data from API
        };
        
        // Store user data and token
        await authStore.login(userData);
        
        console.log('User stored in auth store:', userData);
        
        // Route user based on role
        const redirectPath = routeUserByRole(userRole.role);
        ...
        if (error.message.includes('401') || error.message.includes('Unauthorized')) {
            ...
            loginError.value = 'Access denied. Please contact administrator.';
            ...
            loginError.value = 'Network error. Please check your connection and try again.';
        } else {
  ...
  <div :class="containerClasses">
    ...
    <div class="w-full max-w-md relative z-10">
      ...
      <div :class="cardClasses">
        ...
        <div class="px-8 py-6">
          <!-- General Login Error -->
          <div v-if="loginError" class="mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm">
            {{ loginError }}
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-2">
                <Checkbox v-model="rememberMe" id="remember" binary :disabled="loading" />
          ...
          <div v-if="process.env.NODE_ENV === 'development'" class="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p class="text-sm text-yellow-800 mb-3 font-medium">Quick Login (Development Only):</p>
            <div class="flex flex-wrap gap-2">
              ...
              <Button
                @click="quickLogin('l1@example.com')"
                size="small"
                outlined
                :disabled="loading"
                class="text-xs"
              ...
              <Button
                @click="quickLogin('l2@example.com')"
                size="small"
                outlined
                :disabled="loading"
                class="text-xs"
      ...
      <div v-if="process.env.NODE_ENV === 'development'" class="mt-2 text-center">
